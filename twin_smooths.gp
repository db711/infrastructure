...
twin_smooths(B) = 
{
    ...
    export(multiply);
    start = getwalltime;
    parfor(i=0,2^(length(S))-3,TwinSmoothD(S,2*multiply(vecextract(S,2^(length(S))-1-i)),m,P),r,if(r,res = concat(res,r)));
    res = concat(res,TwinSmoothD(S,2,m,P));
    write("test",vecsort(res));
